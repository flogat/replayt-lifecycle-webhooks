...
- **Automated tests** (e.g. **pytest**) and **ruff** lint/format checks (**`src/`**, **`tests/`**, see **[SPEC_AUTOMATED_TESTS.md](SPEC_AUTOMATED_TESTS.md)** **RF1**–**RF5**) run in **CI** on every change workflow the project uses; the suite **must** cover
...
- **Packaging gate (PyPI readiness)** — Once implemented (**backlog `78e3554b`**), **CI** runs **`python -m build`** and
  **`twine check`** on the produced **sdist** and **wheel**, and verifies **declared package data** (and **`py.typed`** when
  the marker exists in **`src/`**) per **[SPEC_AUTOMATED_TESTS.md](SPEC_AUTOMATED_TESTS.md)** **§ Backlog `78e3554b`**
