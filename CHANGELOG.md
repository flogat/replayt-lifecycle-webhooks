...
- **Replayt version bump guardrails** (phase **2**, backlog **`2b5bb9f6-af49-4c68-a76f-132ec40769d5` / *Contract tests: replayt version bump guardrails*`): **`docs/SPEC_REPLAYT_BOUNDARY_TESTS.md`** adds **§ Version bump guardrails** and acceptance **G1**–**G7** (superset **`__all__`** semantics, actionable failure text naming **SPEC_REPLAYT_DEPENDENCY** + **CHANGELOG**, **network-free** default **`pytest`** vs opt-in markers, optional non-blocking upstream workflow, contributor doc expectations). **`docs/SPEC_AUTOMATED_TESTS.md`** adds backlog **`2b5bb9f6`** **pytest** mapping and ties **§ Minimum behavioral coverage** item **3** to **G1**–**G7**. **`docs/SPEC_REPLAYT_DEPENDENCY.md`** adds traceability and **§ When boundary or guardrail tests fail** maintainer flow.
