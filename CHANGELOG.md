...
- **Offline verify CLI docs** (phase **5**, architect, backlog **`845b4b11-847d-48cb-a9f3-e75f3e4862ef`**): **[`docs/SPEC_AUTOMATED_TESTS.md`](docs/SPEC_AUTOMATED_TESTS.md)**, **[`docs/SPEC_HTTP_SERVER_ENTRYPOINT.md`](docs/SPEC_HTTP_SERVER_ENTRYPOINT.md)**, **[`docs/SPEC_README_OPERATOR_SECTIONS.md`](docs/SPEC_README_OPERATOR_SECTIONS.md)**, **[`docs/SPEC_WEBHOOK_SIGNATURE.md`](docs/SPEC_WEBHOOK_SIGNATURE.md)**, **[`docs/DESIGN_PRINCIPLES.md`](docs/DESIGN_PRINCIPLES.md)** — present-tense and index rows now match the shipped **`verify`** subcommand (no stale “when implemented” for **VW1**–**VW8**).

### Added

- **Offline verify CLI** (phase **3**, backlog **`845b4b11-847d-48cb-a9f3-e75f3e4862ef` / *Support CLI: verify saved webhook (body file + signature header)*`): **`python -m replayt_lifecycle_webhooks verify`** verifies a saved raw body path or stdin (**`-`**) with **`--signature`** or **`--signature-file`**, using **`REPLAYT_LIFECYCLE_WEBHOOK_SECRET`** or **`--secret`**. Exit codes **0** / **1** / **2**; success stdout **`ok\n`**. Tests: **`tests/test_cli_verify_saved_webhook.py`** (**VW1**–**VW8**).
...
- **README metrics hooks** (phase **5**, architect review, backlog **`42b8d5a9-a246-4c47-b167-f39ac371789e` / *Optional metrics hooks for verify / handler outcomes*`): **`README.md`** overview and project layout row link **[`docs/SPEC_METRICS_HOOKS.md`](docs/SPEC_METRICS_HOOKS.md)**. **CHANGELOG** correction: the ASGI recipe bullet above no longer claims a **Phase 5** / **OP1**–**OP9** doc pass that is not part of this branch’s diff against **`master`**.
